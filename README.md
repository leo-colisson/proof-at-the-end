...
- `text proof only theorem`: equivalent of `text proof={\string\pratendRef{thm:prAtEnd\pratendcountercurrent}}`, helpful if the class uses a custom proof environment that would write `Proof (Proof of Lemma X)` instead of `Proof of Lemma X.`: adding this style turns it into `Proof (Lemma X)`. See more details in the troubleshooting section.
